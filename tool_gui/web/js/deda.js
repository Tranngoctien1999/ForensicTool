...
function addField(){
    ...
    input.placeholder = 'Thêm đường dẫn thư mục hoặc tệp ảnh.';
    col1.append(input);
...
function printForensicResult(allresults, errors){
    ...
    if(errors[0].length+errors[1].length>1 || ((errors[0].length>0 || errors[1].length>0) && allresults.length>0)){
        ...
        if(errors[0].length>0){
            u.innerHTML = 'Không có mẫu dấu chấm theo dõi nào được phát hiện. Để có kết quả tốt nhất, hãy thử quét 300 dpi';
            th.innerHTML = 'Files';
            var ferror = '';
            //paths
